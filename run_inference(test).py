...
import json
from utils.ConnectedRegion_np import filter_regions

INPUT_DIR = '/inputs'    # 输入图像文件夹名称
OUTPUT_DIR = '/outputs'  # 输出json文件夹名称
...
def main():
    ...
    file_paths = [os.path.join(INPUT_DIR, entry) for entry in entries]
    ...
    #  导入检查点文件并创造模型
    checkpoint_path = 'your/checkpoints/file'
    ...
    for i, batch in enumerate(test_loader):
        ...
        # 将结果写入json文件
        output_name = os.path.basename(test_images[i]).replace('.jpg', '_Mask.json')
        output_path = os.path.join(OUTPUT_DIR, output_name)
        with open(output_path, 'w') as json_file:
            json.dump(result_dict, json_file, indent=4)
        print(output_path,'ojbk')
