...
class TestingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_path = self.label_paths[index]  # 获取标签文件
        ori_np_mask = cv2.imread(mask_path, 0)  # 转成np数组
        
        if ori_np_mask.max() == 255:
            ori_np_mask = ori_np_mask / 255  # 归一化

        assert np.array_equal(ori_np_mask, ori_np_mask.astype(bool)), f"Mask should only contain binary values 0 and 1. {self.label_paths[index]}"  #确保标签是二值掩码

        h, w = ori_np_mask.shape    # 获取标签的高和宽
        ori_mask = torch.tensor(ori_np_mask).unsqueeze(0)   # 将可能的通道挤压，确保是二维平面数组

        transforms = train_transforms(self.image_size, h, w)    # 获得一种变换方式
        augments = transforms(image=image, mask=ori_np_mask)    # 将图像和标签进行变换
        image, mask = augments['image'], augments['mask'].to(torch.int64)

        if self.prompt_path is None:    # 默认为None
            boxes = get_boxes_from_mask(mask, max_pixel = 0)    # 获得边界框
            point_coords, point_labels = init_point_sampling(mask, self.point_num)  # 从给定的掩膜（mask）中采样点，并为这些点分配标签
        else:
        ...
        image_input["point_coords"] = point_coords  # test 162变None
        image_input["point_labels"] = point_labels  # test 162变None
